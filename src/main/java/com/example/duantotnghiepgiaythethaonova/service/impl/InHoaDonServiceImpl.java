...
public class InHoaDonServiceImpl implements InHoaDonService {
    ...
    @Override
    public ResponseEntity<byte[]> generatePdf(Integer hoaDonId) {
        ...
        if (optHoaDon.isPresent()) {
            ...
            for (HoaDonChiTiet hoaDonChiTiet : hoaDon.getHoaDonChiTiets()) {
                ...
                htmlContentBuilder.append("<td>").append(hoaDonChiTiet.getChiTietSanPham().getSanPham().getTenSanPham())
                        .append(" (").append(hoaDonChiTiet.getChiTietSanPham().getKichCo().getTenKichCo())
                        .append("/").append(hoaDonChiTiet.getChiTietSanPham().getMauSac().getTenMauSac()).append(")")
                        .append("</td>");
    ...
    @Override
    public ResponseEntity<byte[]> generatePdfDonTaiQuay(Integer hoaDonId) {
        Optional<HoaDon> optHoaDon = hoaDonRepository.findById(hoaDonId);
        if (optHoaDon.isPresent()) {
            HoaDon hoaDon = optHoaDon.get();
            // Tạo nội dung HTML cho hóa đơn (thay đổi cho phù hợp với mẫu HTML của bạn)
    ...
    @Override
    public ResponseEntity<byte[]> generatePdfDonDatHang(Integer hoaDonId) {
        Optional<HoaDon> optHoaDon = hoaDonRepository.findById(hoaDonId);
        if (optHoaDon.isPresent()) {
            HoaDon hoaDon = optHoaDon.get();
            // Tạo nội dung HTML cho hóa đơn (thay đổi cho phù hợp với mẫu HTML của bạn)
