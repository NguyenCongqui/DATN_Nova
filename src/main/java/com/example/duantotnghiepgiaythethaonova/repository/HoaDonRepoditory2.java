...
public interface HoaDonRepoditory2 extends PagingAndSortingRepository<HoaDon, Integer> {
    ...
    @Query(value = "SELECT * FROM HoaDon WHERE idTrangThai = 8 and loaiHoaDon = 1 and daXoa = 0 and CONVERT(DATE, ngayTao) =:ngayTao", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM HoaDon WHERE idTrangThai = ?1 AND loaiHoaDon = 0 AND daXoa = 0 AND  CONVERT(DATE, ngayTao) = ?2", nativeQuery = true)
