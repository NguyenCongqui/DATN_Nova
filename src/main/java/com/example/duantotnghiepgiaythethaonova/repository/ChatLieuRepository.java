...
public interface ChatLieuRepository extends JpaRepository<ChatLieu,Integer> {
    @Query(value = "SELECT * FROM ChatLieu c WHERE  c.DaXoa = 0 ORDER BY c.IdChatLieu DESC ",nativeQuery = true)
    Page<ChatLieu> selectAllChatLieuExist(Pageable pageable);

    @Query(value = "SELECT * FROM ChatLieu c WHERE  c.DaXoa = 0 ORDER BY c.IdChatLieu DESC ",nativeQuery = true)
    List<ChatLieu> selectAllChatLieuExist();

    @Query(value = "SELECT * FROM ChatLieu c WHERE  c.DaXoa = 0 AND c.TenChatLieu like %:TenChatLieu% ORDER BY c.IdChatLieu DESC ",nativeQuery = true)
